...
| `state` | `src/state.rs` | `StateFile`, `StateGraph`, `Epic`, `Focus`, `Block`, `Track`, `Backlog`, `Carryover`, `ClearsWhen`, `ClearsWhenPredicate`, `load_state`, `build_state_graph` — the `planning/state.json` schema and its derived graph |
