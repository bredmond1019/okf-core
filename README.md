...
| `CarryoverNeeds`, `KnownCarryoverNeeds` | The optional second axis on a carryover: `kind` says *why* the entry exists, `needs` says *what kind of work closes it* — `code` / `docs` / `state` / `operator` / `dedupe`. Same `Unknown(String)` fallback as `CarryoverKind`. Optional and absent by default, so an entry that never mentions it serializes byte-identically |
