...
2. **Every new function, module, or behaviour change ships with tests.** No exceptions.
3. **OKF frontmatter is required on every new `.md` file under `docs/` and `planning/`.**
4. **No I/O dependencies** — okf-core is a pure leaf library. Do not add filesystem, network, or heavy dependencies. Stick to `serde`, `serde_json`, and `thiserror`. An optional, feature-gated attribute crate that a default build never links is not a violation of this rule — see the `typeshare` feature (`Cargo.toml`), which adds `typeshare` as `optional = true` behind the `typeshare` feature and is absent from a default `cargo tree`.
5. **Coverage bar — separate pure logic from I/O, test the logic exhaustively.**
6. **Decisions are append-only**.
