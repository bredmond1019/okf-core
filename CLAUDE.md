...
The file above carries everything that is true for any agent working in this repo: what this crate
is, the workflow-engine telemetry rule, the standing rules, build/test/run, the directory map, the
response style and the stopping rule. **Read it as part of these instructions** — Claude Code loads
it automatically through the `@` import.
